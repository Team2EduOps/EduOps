...
public class CheckInOut {
	...
	public int showcheckIO(StudentVO stdVo) {
		...
		while (true) {
			...
			if (UtilController.isNull(rs)) {
				System.out.println("executeQuery 실행 중 에러 발생");
				continue;
			}
			try {
				if (!rs.next()) { // 값이 없으면, 입실+결석(0) 모두 "입실" 띄워야함
					checkIo = 0;
					break;
				}
	...
	public void checkIO(StudentVO stdVo) {
		...
		case 1:
			...
			break;
		case 2:
			System.out.println(formattedDate + ": 공가");
			break;
		case 3:
			System.out.println(formattedDate + ": 지각");
			break;
		case 4:
			System.out.println(formattedDate + ": 조퇴");
			break;
		case 5:
			deleteIO(stdVo);
			break;
